...
def parse_meridian_readme_mapping(readme_path: Path) -> dict[str, str]:
    """Return filename -> agent_name mapping for meridian fixtures.
...
async def deactivate_legacy_agents(session: AsyncSession, org_id, active_agent_emails: set[str]) -> None:
    ...
    for user in result.all():
        ...
        if user.is_active:
            ...
            session.add(user)
            print(f"Deactivated legacy Meridian Trust Bank agent: {user.email}")
    await session.commit()
...
async def seed_policies_and_faqs(session: AsyncSession, org_id):
    ...
    storage_docs_dir = backend_dir.parent / "storage" / "docs" / "meridian"
    if not storage_docs_dir.exists():
        storage_docs_dir = Path("/app/storage/docs/meridian")
    ...
    if not policy_files and not sop_files and not kb_files:
        print("No policy, SOP, or KB documents found in storage/docs/meridian.")
        if settings.SEED_MOCK_INTERACTIONS:
            return await seed_dummy_policy(session, org_id)
        return None
...
async def seed_dummy_policy(session: AsyncSession, org_id):
    ...
    result = await session.exec(select(CompanyPolicy).where(CompanyPolicy.policy_title == "Meridian Trust Bank Refund Policy"))
    policy = result.first()
    if not policy:
        policy = CompanyPolicy(
            organization_id=org_id,
            policy_category="Guidelines",
            policy_title="Meridian Trust Bank Refund Policy",
...
async def seed_interactions(
    ...
    # Scan ../storage/audio/meridian for files
    backend_dir = Path(__file__).resolve().parents[1]
    audio_dir = backend_dir.parent / "storage" / "audio" / "meridian"
    if not audio_dir.exists():
        print(f"Audio directory not found: {audio_dir}")
        return

    audio_files = list(audio_dir.glob("*.mp3")) + list(audio_dir.glob("*.wav"))
    readme_mapping = parse_meridian_readme_mapping(audio_dir / "README.md")
    ...
    if not audio_files:
        print("No audio files found in storage/audio/meridian")
        return

    print(f"Found {len(audio_files)} audio files. Creating interactions...")

    for path in audio_files:
        filename = str(Path("..") / "storage" / "audio" / "meridian" / path.name)
    ...
    print("\nMeridian Trust Bank assignment summary:")
    for agent in agents:
...
async def main():
    async with AsyncSession(engine, expire_on_commit=False) as session:
        print("Starting Database Seed for Meridian Trust Bank...")
        org = await create_or_get_organization(session, "Meridian Trust Bank", "meridian")
        
        manager = await create_or_get_user(session, org.id, "manager@meridian.com", "Meridian Trust Bank Manager", UserRole.manager)
        agents = [
            await create_or_get_user(session, org.id, email, name, UserRole.agent, AgentType.human)
            for email, name in MERIDIAN_AGENT_PROFILES
        ]
        await deactivate_legacy_agents(session, org.id, {email for email, _ in MERIDIAN_AGENT_PROFILES})
        ...
        print("Manager: manager@meridian.com / password123")
        for email, _ in MERIDIAN_AGENT_PROFILES:
